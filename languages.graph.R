...
# Edgelist with weighted co-occurrences by language----
  ...
  ## Edgelist considering who speaks which language: i.e, Pedro and Juan (German) ≠ Pedro and Juan (Spanish). If characters (nodes) speak different languages between them, we get multiple edges (also called parallel edges or a multi-edges): the resulting graph is then a multi-graph. 
...
# Metrics----
  ...
  V(graph)$art.point[V(graph)$art.point %in% cut_nodes] = 1 # 1 == they are articulation points
  V(graph)$art.point[V(graph)$art.point != 1] =  0 # 0 == they are not articulation points

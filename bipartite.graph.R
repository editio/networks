...
# Bipartite graph of characters and languages spoken by each of them.
...
# After runing it, variables are strored in the "graph"

# Load data----
interaction.table <- read.csv("csv/redes.semprilis.csv", na.strings="")
interaction.table <- data.frame(lapply(interaction.table, str_trim)) # str_trim() removes whitespace from start and end of string. " Semprilis " ≠ "Semprilis".
...
V(graph)$type <- V(graph)$name %in% edgelist[,2]  # Match nodes names between graph and edgelist (character column), and adds the info (true/false) as an atributte to the graph object.

# Calculates the edges weight----
# get.incidence(graph): creates an incidences matrix with weighted edges. 
